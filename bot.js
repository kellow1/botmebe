...
/*
   client.user.setPresence({ game: { name: 'comando', type: 1, url: 'EM BREVE'} });
    //0 = Jogando
    //  1 = Transmitindo
    //  2 = Ouvindo
    //  3 = Assistindo
...
client.on("message", async message => {
  ...
  if(comando === "ping") {
    ...
    m.edit(`Latencia do PVP ${m.createdTimestamp - message.createdTimestamp}ms. Latencia BR ${Math.round(client.ping)}ms`);
  }
  //comando falar
  if(comando === "say") { 
...
//comando apagar
  if(comando === "apagar") {
    ...
    if(!deleteCount || deleteCount < 2 || deleteCount > 500)
      return message.reply("Por favor, forneça um número entre 2 e 500 para o número de mensagens a serem excluídas");
...
//adicione o nome dos cargos que vc quer que use esse comando!
    if(!message.member.roles.some(r=>["🌹│Fundador", "💥│Owner"].includes(r.name)) )
